// Clash/Mihomo Override Script v1.2.0 | WebRTC + DNS + Regional + Process Routing
const CONFIG = {
  PRESET: "balanced",
  DEBUG: false,
  ...
  // 🆕 WebRTC Leak Protection (Default OFF to avoid breaking video calls)
...
// ===== DNS Configuration (Mihomo Optimized) =====
const dnsConfig = {
  enable: true,
...
function main(config){
  ...
  console.log(`[Override] v1.2.0 start | preset:${CONFIG.PRESET}`);

  // ===== Core Stack Injection =====
  ...
  console.log(`[Override] v1.2.0 | Nodes:${config.proxies.length} | Rules:${rules.length} | Groups:${groups.length}`);
  if(CONFIG.DEBUG){
    console.log(`[Debug] === v1.2.0 Report ===`);
